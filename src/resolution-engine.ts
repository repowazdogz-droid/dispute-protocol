...
export function proposeResolution(
  ...
  options?: { harm_minimising_party_id?: string }
): Resolution {
  ...
  switch (method) {
    ...
    case 'precedent': {
      const similar = precedents.filter(
        (pr) => pr.category === dispute.category && pr.applicable_domains.length > 0
      );
      if (similar.length > 0) {
        const pr = similar[0];
